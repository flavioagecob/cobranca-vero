  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      ...
      if (filters.parcela) {
        ...
        // ALSO apply safra and statusContrato filters
        let parcelaContracts = (allContracts || []).filter(c => c.numero_fatura === filters.parcela);
        
        if (filters.safra) {
          parcelaContracts = parcelaContracts.filter(c => c.mes_safra_cadastro === filters.safra);
        }
        
        if (filters.statusContrato) {
          parcelaContracts = parcelaContracts.filter(
            c => c.status_contrato?.toLowerCase() === filters.statusContrato.toLowerCase()
          );
        }
        ...
        Object.entries(filteredContractsByCustomer).forEach(([customerId, contracts]) => {
          const allPaid = contracts!.every(c => c.data_pagamento !== null);
          const hasOverdueInvoices = contracts!.some(c => {
            if (c.data_pagamento !== null) return false;
            if (!c.data_vencimento) return false;
            return c.data_vencimento < todayStr;
          ...
          if (allPaid) {
            filteredCustomerSituacaoMap[customerId] = 'paid';
          } else if (hasOverdueInvoices) {
            filteredCustomerSituacaoMap[customerId] = 'overdue';
          } else {
            // Has pending but not overdue invoices - consider as "paid" (em dia)
            filteredCustomerSituacaoMap[customerId] = 'paid';
          }
        });

        // Get unique customer IDs from filtered contracts
        const uniqueCustomerIds = Object.keys(filteredContractsByCustomer);
        ...
        if (hasContractFilter) {
          // When filtering by contracts, count unique customers from the filtered set
          ...
          setStats({
            total: uniqueCustomerIds.length, // UNIQUE CUSTOMERS with filtered contracts
            paid: paidCount,
